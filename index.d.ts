...
declare module 'arcgoose' {
    ...
    interface Find<T, K> {
         ...
         * ```
         */
        sort(sortOrder:  PartialTyped<T, number>): Find<T, K>
     ...
     * }
     * 
     * const connection = await arcgoose.connect<FeatureServiceDefinition>({
     ...
     * @example ```typescript
     * 
     * // `ServiceDefinition` should be an interface
     * const connection = await arcgoose.connect<FeatureServiceDefinition>({
     *      portalUrl: 'https://www.arcgis.com/sharing/rest',
     *      portalItemId: '257fb60e3da45789817b3d69aee88a2'
     * })
     * ```
     */
    export function connect<T>(config: { url: string } | {portalUrl: string, portalItemId: string}): Connection & T;
     ...
     * const model = await arcgoose.model<Cat>(
     *      connection.layers.Cats, 
     *      catSchema
     * )
     * ```
     */
